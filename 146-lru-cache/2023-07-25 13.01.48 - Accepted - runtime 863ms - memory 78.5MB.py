...
class LRUCache:
    ...
    # Insert the node with specified key and value to the second last right of the DLL
    def insert(self, node: Node):
        prev, last = self.right.prev, self.right
        prev.next = last.prev = node
        node.next, node.prev = last, prev

    # Remove the node with specified key from DLL 
